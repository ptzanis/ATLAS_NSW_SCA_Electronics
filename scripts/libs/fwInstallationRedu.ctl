 ...
 * @ingroup Constants
*/
const string csFwInstallationReduLibVersion = "8.2.0";
...
bool fwInstallationRedu_isRedundant()
{
  ...
  if (event != "")
  {
    isRedu = strpos(event, FW_INSTALLATION_CONFIG_REDU_SYS_DELIMITER) > 0;
  }
  else
  {
    isRedu = isRedundant();
...
int fwInstallationRedu_myReduHostNum()
{
  ...
  dyn_dyn_int ports;
  if(eventHostName == "" || fwInstallation_parseHostPortConfigEntry(eventHostName, hostAliases, ports) != 0)
  {
    fwInstallation_throw("Could not retrieve event hosts/aliases from config file, using internal WinCC OA function to get redu host number" +
                         "(in non-redu configurations it always returns 1)", "WARNING");
    return myReduHostNum();
  }
  
  string localHostName = fwInstallation_getHostname();
  int peersCount = dynlen(hostAliases);
  // Try to match host given in event config entry with current hostname directly. Note: Does not cover all cases, but it's faster than resolving hostname from alias.
  for(int peerNum=1;peerNum<=peersCount;peerNum++)
  {
    if(dynlen(hostAliases[peerNum]) > 0 &&
       strtoupper(localHostName) == strtoupper(fwInstallation_getHostname(hostAliases[peerNum][1])))
    {
      return peerNum;
    }
  }
  
  // Fallback solution, resolve hostname from config event entry alias. Note: might introduce a delay until host will be determined.
  for(int peerNum=1;peerNum<=peersCount;peerNum++)
  {
    int aliasesCount = dynlen(hostAliases[peerNum]);
    for(int j=1;j<=aliasesCount;j++)
    {
      if(fwInstallationRedu_matchAliasWithHostName(hostAliases[peerNum][j], localHostName))
      {
        return peerNum;
      }
    }
  }
  fwInstallation_throw("Could not match local host name with any event manager config entry host/alias. Returning 1", "WARNING");
  return 1;
}
