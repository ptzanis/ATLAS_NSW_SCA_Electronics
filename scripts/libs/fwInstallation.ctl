 ...
 * @ingroup Constants
*/
const string csFwInstallationToolVersion = "8.2.0";
/** Version of this library.
 * Used to determine the coherency of all libraries of the installtion tool
 * @ingroup Constants
*/
const string csFwInstallationLibVersion = "8.2.0";
                                       ...
                                       string &dontRestartProject)
{
  ...
  for (int i =1; i <= dynlen(componentFilesInOrder); i++)
  {
    ...
    retVal = fwInstallation_installComponent(componentFilesInOrder[i],
                                           sourceDir,
                                           isSubcomponent,
                                           componentName,
                                           componentInstalled,
...
/** This function parses the config entries that contains host name and port ('data', 'event', part of 'distPeer' within quotes).
  ...
  * Example entries:
  * "host1[:port1]"
  * "host1[:port1]$host2[:port2]" // redundant system
  * "host1-1[:port1],host1-2[:port1]" // redundant network connections
...
int fwInstallation_parseHostPortConfigEntry(string configHostEntry, dyn_dyn_string &hostAliases, dyn_dyn_int &ports)
{
  ...
  for(int i=1;i<=reduPeersLen;i++)
  {
    ...
    int reduPeerConnsLen = dynlen(reduPeerConns);
    for(int j=1;j<=reduPeerConnsLen;j++)
    {
...
void fwInstallation_addLibToConfig()
{
  ...
  paCfgReadValueList(PROJ_PATH + CONFIG_REL_PATH + FW_INSTALLATION_CONFIG_FILE_NAME, "ui", "LoadCtrlLibs", libs);
  if(dynContains(libs, "fwInstallation.ctl") == 0)
    paCfgInsertValue(PROJ_PATH + CONFIG_REL_PATH + FW_INSTALLATION_CONFIG_FILE_NAME, "ui", "LoadCtrlLibs", "fwInstallation.ctl");

  paCfgReadValueList(PROJ_PATH + CONFIG_REL_PATH + FW_INSTALLATION_CONFIG_FILE_NAME, "ctrl", "LoadCtrlLibs", libs);
  if(dynContains(libs, "fwInstallation.ctl") == 0)
    paCfgInsertValue(PROJ_PATH + CONFIG_REL_PATH + FW_INSTALLATION_CONFIG_FILE_NAME, "ctrl", "LoadCtrlLibs", "fwInstallation.ctl");
}
...
int fwInstallation_updateDPT()
{
  ...
  bool postInitRestartNeededDpeExists = dpExists(fwInstallation_getAgentDp() + "." + fwInstallation_getAfterInitRestartNeededDpElem());
  bool synchronizedComponentsDpeExists = dpExists(fwInstallation_getAgentDp() + FW_INSTALLATION_DB_AGENT_SYNC_COMPONENTS_DPE);
   
  if(!postInitRestartNeededDpeExists || !synchronizedComponentsDpeExists)
  {
    if(_fwInstallation_createAgentDataPointType("_FwInstallation_agentParametrization", true) != 0){
      fwInstallation_throw("Failed to create/modify _FwInstallation_agentParametrization datapoint type, aborting...", "ERROR");
      return -1;
    }
    
    if(!postInitRestartNeededDpeExists){
      int dpCreateResult = 0;
      _fwInstallation_createDataPointTypes(FALSE, dpCreateResult); //Not up-to-date: Update dpts now
      if(dpCreateResult){
        fwInstallation_throw("Failed to create/modify _FwInstallationComponents and/or _FwInstallationInformation datapoint type");
        return -1;
      }
    ...
    // Set initial value to syncronizationComponents local and central dpe (note: it will be an array with ones)
    dyn_bool syncCompsList;
    if(fwInstallationDBAgent_setEnabledSynchronizationComponents(true, syncCompsList) != 0 ||
       fwInstallationDBAgent_setEnabledSynchronizationComponents(false, syncCompsList) != 0)
    {
...
//	   dynDynElements[12] = makeDynString ("", "", "synchronization");
    ...
    dynDynElements[29] = makeDynString ("", "synchronizedComponents");
      dynDynElements[30] = makeDynString ("", "", "enabledInLocal");
      dynDynElements[31] = makeDynString ("", "", "enabledInCentral");
...
//    dynDynTypes[12] = makeDynInt (0, 0, DPEL_DYN_INT);
    ...
    dynDynTypes[29] = makeDynInt(0, DPEL_STRUCT);
    dynDynTypes[30] = makeDynInt(0, 0, DPEL_DYN_BOOL);
    dynDynTypes[31] = makeDynInt(0, 0, DPEL_DYN_BOOL);
  }
...
/** The function reads all project paths from the config file into a dyn_string list.
...
int fwInstallation_getProjPaths(dyn_string& projPaths, bool adjustScattered = false)
{
  ...
  int retVal = paCfgReadValueList(configFile, "general", "proj_path", projPaths);
...
int fwInstallation_backupProjectConfigFile()
{
  ...
  string configFile = configPath + FW_INSTALLATION_CONFIG_FILE_NAME;
  ...
  string bkConfigFile = configPath + FW_INSTALLATION_CONFIG_FILE_NAME + str;
...
fwInstallation_showHelpFile(string componentName, string systemName = "")
{
  ...
  string helpFile = componentInstallationPath + "/help/en_US.iso88591/" + helpFileRelativePath;
...
int fwInstallation_importConfigFilesOfType(string componentName, string sourceDir, string subPath, const dyn_string &dynConfigFiles, string typeLabel)
{
  ...
  for(int i=1;i<=dynConfigFilesLen;i++)
  {
    ...
    bool shallAddToConfigRedu = (strtolower(substr(configFilePath, strlen(configFilePath) - 5)) == ".redu");
...
//			strreplace(dynInstalledComponents[i], "fwInstallation_" , "");	
    ...
    dynInstalledComponents[i] = name;
     ...
     string reqDp = fwInstallation_getComponentDp(strRequiredName);
				 ...
				 if(_fwInstallation_CompareVersions(stringInstalledVersion, stringRequiredVersion, false, false, true))
				 {
...
const string FW_INSTALLATION_CONFIG_COMPONENT_TAG_BEGIN = "#begin ";
const string FW_INSTALLATION_CONFIG_COMPONENT_TAG_END = "#end ";
...
{
  const string configComponentTagBegin = FW_INSTALLATION_CONFIG_COMPONENT_TAG_BEGIN + componentName;
  const string configComponentTagEnd = FW_INSTALLATION_CONFIG_COMPONENT_TAG_END + componentName;
  ...
  for(int i=configLinesLen;i>=1;i--)
  {
    ...
    if(configLine == configComponentTagEnd){ // Iterate over config lines in reverse order so component part starts when "#end [componentName]" tag is found
      componentLineEnd = i;
    }

    if(configLine == configComponentTagBegin){ // Component part ends when "#begin [componentName]" tag is found (reverse order)
...
/** Retrieves ID of the section in the memory representation of config file.
  * If section is not yet there, appends it and initializes corresponfing indexes in configKeysArray and configValuesArray.
...
{ // regexp: match word charactes (letter, number, mark, underscore) enclosed by square brackets (including brackets) followed by 0 or more whitespace characters and end of line or comment character
  const string SECTION_REGEXP = "^\\" + FW_INSTALLATION_CONFIG_SECTION_BEGIN_CHAR + "\\w+\\" + FW_INSTALLATION_CONFIG_SECTION_END_CHAR +
                               "\\s*($|" + FW_INSTALLATION_CONFIG_COMMENT_CHAR + ")"; // ^\[\w+\]\s*($|#)
    ...
    switch(fwInstallation_parseConfigLine(componentConfigLines[i], key, keyValues)){
      case FW_INSTALLATION_CONFIG_SECTION_LINE_CODE:
        if(projectSectionId > 0){ // Put component end tag in previous section (if there was any)
          fwInstallation_config_insertComponentTag(projectConfigLines, projectConfigLineNum, componentName, FW_INSTALLATION_CONFIG_COMPONENT_TAG_END);
        }
        ...
        fwInstallation_config_insertComponentTag(projectConfigLines, projectConfigLineNum, componentName, FW_INSTALLATION_CONFIG_COMPONENT_TAG_BEGIN);
        break;
...
/** This function inserts the component tag into the config lines (information where the component parametrization starts/ends).
  ...
  * @param lineNumber (in/out)  Line number where the tag should be inserted, note that line number gets updated after insertion
  * @param componentName (in)  Name of the component
  * @param tagType (in)  String indicating if the begin or end tag should be inserted, two constants are allowed:
  *                      FW_INSTALLATION_CONFIG_COMPONENT_TAG_BEGIN and FW_INSTALLATION_CONFIG_COMPONENT_TAG_END
  */
void fwInstallation_config_insertComponentTag(dyn_string &configLines, int &lineNumber, string componentName, string tagType)
{
  switch(tagType){
    case FW_INSTALLATION_CONFIG_COMPONENT_TAG_BEGIN:
      dynInsertAt(configLines, FW_INSTALLATION_CONFIG_COMPONENT_TAG_BEGIN + componentName, lineNumber++);
      dynInsertAt(configLines, "#This should not be edited manually", lineNumber++);
      dynInsertAt(configLines, "#if the component is empty it means that the parametrization is already done in the section", lineNumber++);
      break;
    case FW_INSTALLATION_CONFIG_COMPONENT_TAG_END:
      dynInsertAt(configLines, FW_INSTALLATION_CONFIG_COMPONENT_TAG_END + componentName, lineNumber++);
      break;
    default:
      fwInstallation_throw("Cannot insert component tag, unrecognized tag type: " + tagType);
  }
}
  ...
  for(int jj = 1; jj <= dynlen(requiredComponents); jj++){
    ...
    for(int ii = 1; ii <= dynlen(componentsInfo);ii++){
      ...
      if(patternMatch(requiredComponentName, fileComponentName)){
        ...
        if(_fwInstallation_CompareVersions(fileComponentVersion, requiredComponentVersion, false, false, true)){
                                                  ...
                                                  bool hiddenComponentsVisible, string systemName = "")
{
  ...
  tblAvailableComponents.appendLines(availableComponentsNumber, 
  ...
  fwInstallation_ui_filterAvailableComponentsTable(tableShape, subcomponentsVisible, hiddenComponentsVisible);
}

/** Show/hide rows with subcomponents and hidden component in Available components table according to given parameters.
  * @param tableShape (in)  Available components table shape
  * @param subcomponentsVisible (in)  Flag that indicates if subcomponents should be shown (true) or hidden
  * @param hiddenComponentsVisible (in)  Flag that indicates if hidden components should be shown (true) or hidden
  */
void fwInstallation_ui_filterAvailableComponentsTable(shape tableShape, bool subcomponentsVisible, bool hiddenComponentsVisible)
{
...
/** Reads all rows of the given table's columns as a string.
  ...
  * @param onlyVisible (in)  Flag that indicates if only content of visible rows should be read
...
string fwInstallation_ui_getTableColumnsAsString(shape tableShape, dyn_string columns, bool addHeader = true, string colSeparator = "\t", bool onlyVisible = true)
{
  ...
  for(int i=0;i<tableRows;i++) {
    if(onlyVisible && tableShape.isRowHidden(i)){
      continue; // If onlyVisible flag is set skip hidden rows
    }
    for(int j=1;j<=columnsToGet;j++)
    {
